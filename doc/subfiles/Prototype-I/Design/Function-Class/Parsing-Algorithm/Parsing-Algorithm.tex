...
\Fn{checkBracket(String input)}{
	...
	\While{!done}{
		...
		\uIf{input[0] == `(' \KwAnd input[input.size - 1] == `)'}{
			...
			\eIf(\tcc*[h]{we haven't looped til the last character which is `)' , and therefore at this point countMatching would be 1 for a standard expression}){countMatching == 1}{
				done = False\;
				input = input.subString(1, input.size - 2)\;
			}{
				\tcc*[h]{if not it is not an accepted expression, unequal brackets}\;
