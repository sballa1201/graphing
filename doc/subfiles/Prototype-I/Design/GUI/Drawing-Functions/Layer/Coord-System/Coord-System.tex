...
\end{algorithm}
Now that we have the pixel worth, we can convert between our coordinate systems. The following algorithms convert between $x$ and $y$ coordinate\footnote{Here we assume the coordinate is inside the viewport, we can make checks for this later on} repectively:

\begin{algorithm}[H]
\DontPrintSemicolon
\caption{Convert the $x$ value}
\Fn{convertX(\KwDouble x)}{
	x = x - minX \tcc{Calculate the absolute distance between the coord and origin}
...
\begin{algorithm}[H]
\DontPrintSemicolon
\caption{Convert the $y$ value}
\Fn{convertX(\KwDouble y)}{
	y = maxY - y \tcc{Calculate the absolute distance between the coord and origin}
